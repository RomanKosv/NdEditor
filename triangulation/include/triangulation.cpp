...
std::optional<Segment1d> Segment1d::cutMore(Scalar cut) {
    if (cut>right) {
...
std::optional<Segment1d> Segment1d::cutLess(Scalar cut) {
    if (cut < left) return std::nullopt;
    else return Segment1d{left, std::min(right, cut)};
}

std::optional<Segment3d> Segment3d::cutLess(Vec3d axis, Scalar cut) {
    Scalar cos = scalar_mult(axis, vec)/(len(axis)*len(vec));
    if (cos == 0) {
        LOG <<" cos = 0\n";
        ...
        LOG << "cos is" <<cos << '\n';
        Scalar point_c = scalar_mult(axis, point)/len(axis);
        Scalar point_d = cut/len(axis) - point_c;
...
std::optional<Vec3d> Segment3d::getA() {
    if (limits.left != N_INF) return sum(point, scale(vec, limits.left/len(vec)));
    else return std::nullopt;
}
std::optional<Vec3d> Segment3d::getB() {
    if (limits.right != P_INF) return sum(point, scale(vec, limits.right/len(vec)));
    else return std::nullopt;
}
std::optional<Segment3d> Segment3d::cut(std::optional<Segment3d> seg, Halfspace3d cut) {
...
std::variant<Segment3d, IntersectionError> intersection(Halfspace3d& h1, Halfspace3d& h2) {
    ...
    Vec3d vec1 = scale(axis1, c1 / len2(axis1));
    Vec3d vec2 = scale(axis2, c2 / len2( axis2));
    if (scalar_mult(axis1, axis2)*scalar_mult(axis1, axis2) == len2(axis1)*len2(axis2)) {
        ...
        Vec3d vec_1_without_2 = sum(vec1, scale(axis2, -scalar_mult(axis1, axis2)*len(vec1)/(len(axis1)*len2(axis2))));
    ...
    for(size_t pl_i = 1; pl_i < cuts.size(); pl_i++) {
        ...
        for(size_t cutter_i = 0; cutter_i < planes.size() - 1; cutter_i++) {
            ...
            if (cut_rez.index() == 1)  {
                ...
                LOG << "sefment from" << pl_i<<'x' << cutter_i << ' ' << str(get<Segment3d>(cut_rez)).data() << '\n';
            ...
            if (pl2_i != pl_i+1)
                for(size_t i = 0; i < segments[pl_i].size(); i++) {
                    if (i!= pl2_i){
                        ...
                        LOG <<"segment" <<pl_i <<"::" <<i <<str(segments[pl_i][i]).data() <<" cut by" <<pl2_i <<'\n';
                    }
                }
        }
    }
    int i = 0;
    for(auto pl : segments) {
        int j = 0;
        for(auto s : pl) {
            LOG << "[segment " << i << 'x' << j++ << ']' << str(s).data();
        }
        i++;
        LOG << '\n';
    }
        ...
        for(int p = 1;p<points.size(); p++){
            if (/*!checkSquare || !(std::get<0>(points[0]) == std::get<0>(points[p]) || std::get<0>(points[0]) == std::get<1>(points[p]) || std::get<0>(points[p]) == std::get<1>(points[p]))*/ true) {
                LOG << "[Triangle by plane"<<i << "]"
                    <<str(std::get<0>(points[0])).data()
                    <<str(std::get<0>(points[p])).data()
                    <<str(std::get<1>(points[p])).data() <<'\n';
                triangles.push_back({
                                     std::get<0>(points[0]),
                                     std::get<0>(points[p]),
                                     std::get<1>(points[p])});
